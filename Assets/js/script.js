...
const storage = (() => {
    const addWord = () => {
        ...
        var addConfirmBox = confirm("Do You Want To Add This To Your List?");
        if (addConfirmBox) {
    ...
    const getWordFromStorage = () => {
        var selectedWord = searchedWord; // Not true, should be word selected from review
        var deleteConfirmBox = confirm("Are You Sure, You Want To Delete This Word?");
        if (deleteConfirmBox) {
            if (addedWords.length > 0) {
...
const quizcard = (() => {
    ...
    const generateQuiz = () => {
        ...
        for (
            var i = 0, length = fetchedAddedWordsFromLocalStorage.length; i < length; i++
        ) {
